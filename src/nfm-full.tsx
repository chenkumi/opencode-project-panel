...
import { readdirSync, readFileSync, openSync, readSync, closeSync, mkdirSync, writeFileSync, renameSync, rmSync } from "node:fs"
...
export default function NewFileManager(props: Props) {
  ...
  useBindings(() => ({
    ...
    bindings: [
      ...
      {
        key: "return",
        cmd: () => (overlayConfirmAction ?? hideOverlay)(),
      },
    ],
  }))

  useBindings(() => ({
    enabled: () => overlayMode === "prompt",
    priority: 2,
    bindings: [
      {
        key: "escape",
      ...
      },
      {
        key: "return",
        cmd: () => {
          overlayPromptAction?.(promptInputValue)
        },
      },
    ],
  }))
  ...
  useBindings(() => ({
    ...
    priority: 10,
    bindings: [
      { key: "f7", cmd: handleCreate },
      { key: "f2", cmd: handleRename },
      { key: "ctrl+r", cmd: handleRename },

      { key: "delete", cmd: handleDelete },
      { key: "ctrl+f", cmd: () => filterInputEl?.focus() },
    ],
  }))

  useBindings(() => ({
    target: filterInputTarget,
    enabled: () => filterInputTarget() !== undefined,
    priority: 1,
    bindings: [
      {
        key: "escape",
  ...
  return (
    <box width={panelWidth()} height={panelHeight()} flexDirection="column">
      ...
      <box paddingLeft={2} paddingTop={1} paddingBottom={1} flexShrink={0} flexDirection="row" gap={1}>
