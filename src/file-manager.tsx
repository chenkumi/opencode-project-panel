...
export type FileAction = "create" | "rename" | "delete" | "goto" | "reload" | "returnToProjectRoot" | "focusFilter" | "focusSelect" | "hideOverlay"
...
export default function NewFileManager(props: Props) {
    ...
    function navigateTo(dir: string) {
        ...
        pathTextEl.content = projectPathText(currentDir)
        forceSelectFirst()
    ...
    function handleReload() {
        ...
        const selectedIndex = filteredEntries.findIndex((e) => e.fullPath === selectedPath)
        if (selectedIndex >= 0) {
            selectRef.setSelectedIndex(selectedIndex)
            currentEntry = filteredEntries[selectedIndex] ?? null
        } else {
            currentEntry = filteredEntries[0] ?? null
    ...
    const projectPathText = (dir: string) => {
        const prefix = "Project Path: "
        const available = Math.max(panelWidth() - 4 - prefix.length, 1)
        return prefix + truncateText(dir, available)
    }
        ...
        <box width={panelWidth()} height={panelHeight()} flexDirection="column">
            <box paddingLeft={2} paddingRight={2} flexShrink={0} border={["bottom"]} borderColor={theme().textMuted} flexDirection="column">
                <box height={1} flexShrink={0} flexDirection="row" width="100%">
                    <text ref={(el) => { pathTextEl = el }} fg={theme().text} wrapMode="none">
                        {projectPathText(currentDir)}
                    </text>
                </box>
                <box height={1} flexShrink={0} flexDirection="row" width="100%">
                    <box flexDirection="row" gap={3}>
                        ...
                        <box flexDirection="row" gap={0} onMouseUp={() => handleReload()}>
                ...
                <box ref={(el) => { mainContentBox = el }} flexDirection="row" flexGrow={1}>
                    <select
                        ...
                        onMouseDown={(event: any) => {
                            ...
                            const visibleIdx = Math.floor(event.y - selectRef.screenY)
                            const scrollOffset = (selectRef as any).scrollOffset ?? 0
                            const idx = scrollOffset + visibleIdx
                            if (visibleIdx >= 0 && idx < filteredEntries.length) {
                                const now = Date.now()
